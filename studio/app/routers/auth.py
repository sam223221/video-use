...
* ``POST /api/login``  {username, password} -> validated against the configured
  multi-user map; on success sets a signed httpOnly session cookie recording
  WHICH username authenticated. **Scheme-aware since Secure Studio
  (2026-06-11):** a login over plain http issues the legacy ``studio_session``
  cookie exactly as before (SameSite=Lax, NOT Secure); a login over https
  issues ``__Host-studio_session`` instead (Secure + Path=/ + NO Domain — the
  ``__Host-`` prefix integrity rules mean an insecure origin can never plant
  or overwrite it). Same HMAC token format either way. The request scheme is
  trustworthy here because uvicorn serves both listeners directly (no proxy).
  On failure: one indistinguishable 401 ``{"ok": false, "error": "invalid
  credentials"}`` for both unknown-user and wrong-password (no enumeration).
* ``POST /api/logout`` -> clears BOTH cookies (legacy + ``__Host-``).
* ``GET  /api/me``     -> ``{"authenticated": bool, "username": str|null,
  "stale_sessions": int, "asset_version": str|null, "secure_url": str|null}``
  — PUBLIC (used by the frontend to gate the UI); reflects the cookie's
  username, the COUNT of the caller's stale sessions (idle > 14 days; full
  list at ``GET /api/sessions``), the frontend ASSET_VERSION the server is
  serving (parsed read-only from ``frontend/app.js``, mtime-cached; ``null``
  on any parse failure) so a long-lived tab can self-detect stale JS, and
  ``secure_url`` — the https LAN URL when TLS is up (drives the SPA's
  ``#secure-banner``), ``null`` when TLS is off/unavailable.

NO HSTS anywhere (deliberate, plan §6): Strict-Transport-Security would
force-upgrade the plain-http bootstrap origin that first-contact onboarding
depends on.

Logging policy (diagnostic-logging plan, 2026-06-10): login success/failure and
...
session tokens), and the failed-login username is control-stripped + capped so
a crafted value cannot forge log lines. Credentials are compared in constant
...
_log = logging.getLogger("studio.auth")
...
def _set_session_cookie(
    ...
    #
    # secure=True (an https login): issue __Host-studio_session instead. The
    ...
    # here. The legacy http cookie stays byte-identical to before (the
...
def login(body: LoginBody, request: Request) -> JSONResponse:
    ...
    # the attempted username + IP per the diagnostic-logging plan, but NEVER
    # the password).
    ...
    # The scheme decides the cookie name (Secure Studio): direct uvicorn, no
    # proxy, so request.url.scheme is ground truth. Logged for diagnostics —
    # never the token.
...
def logout(request: Request) -> JSONResponse:
    ...
    # and the cookie would survive logout.
...
def me(request: Request) -> JSONResponse:
    ...
    return JSONResponse(
        content={
            ...
            # The https LAN URL when TLS is up (drives the SPA's secure-setup
