"""POST /api/client-log — phone-side diagnostics ingestion (plan 2026-06-10).

The live incident (phone uploads stalling in total client silence) cannot be
diagnosed from server evidence alone — the phone has to tell us what its
uploader was doing. The frontend's ``diag.js`` ring-buffers events and ships
them here in small batches (plus a ``sendBeacon`` tail on pagehide); each event
becomes ONE line in the rotating server log (``studio.client`` logger):

    client user=<username> ip=<ip> ua="<first-80-of-user-agent>" | <level> t=<epoch_ms> <msg> <data>

Contract (frozen in the Feature Plan §1.A):
...
* A garbage body is a clean 400 ``invalid_body`` envelope — never a 500. The
  body is parsed manually (not via a Pydantic model) precisely so the size cap
  runs against the RAW bytes and malformed JSON maps to the contract's 400
  instead of FastAPI's 422 validation array.
* Log-line injection is impossible: every client string is passed through
  ``applog.sanitize_log_value`` (C0/C1 control chars — incl. CR/LF — replaced,
  length-capped), and events are written as DATA via the logging framework,
  never interpolated into anything executable.
* Response: 200 ``{ok: true}``.

Content-Type is deliberately NOT restricted to ``application/json``: the
``navigator.sendBeacon`` fallback (the dying-tab tail — the single most
valuable batch in this incident) may ship as ``text/plain`` depending on how
the Blob is constructed, and losing that tail would defeat the feature. The
CSRF exposure this opens is writing rate-capped, sanitized NOISE into a
private diagnostic log under the victim's own username — no state changes, no
reflection, bounded size — accepted and documented here.
...
_log = logging.getLogger("studio.client")
...
# studio logger filters below INFO, and dropping the client's debug events
...
async def client_log(
    ...
    try:
        ...
        # mirror the upload contract's resumable 499 rather than a 500.
        raise deps.http_error(
