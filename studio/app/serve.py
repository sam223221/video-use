"""Dual-listener launcher (Secure Studio plan, 2026-06-11).

``python -m app.serve`` (cwd = ``studio/`` — exactly how ``start.bat`` /
...
* **HTTP**  on ``settings.HOST:settings.PORT``  — lifespan ON ("auto", the
  CLI default): migration / GC / husk sweep / banner run here, exactly once,
  byte-identical to the old ``python -m uvicorn app.main:app`` invocation.
* **HTTPS** on ``settings.HOST:settings.TLS_PORT`` — ``ssl_certfile`` /
  ``ssl_keyfile`` from :func:`app.core.tls.ensure_certs`, ``lifespan="off"``
  so startup logic never runs twice.

ONE process is load-bearing: the upload registry, agent-client cache, job
registry, SSE bus, and the rotating log handler are all in-memory singletons —
two processes would split them (and Windows would contend on the log file).

Fallbacks (TLS can NEVER block boot — the applog invariant):
* ``STUDIO_TLS=0`` (or ``[server] tls=false``) → single HTTP server, exactly
  like today.
* ``ensure_certs()`` → ``None`` (any cert failure) → same HTTP-only fallback,
  loudly logged.
* The HTTPS listener failing at runtime (e.g. ``:8443`` already bound) is
  contained: the HTTP server keeps serving and ``/api/tls/info`` flips to
  ``enabled:false``.

Signals / Ctrl+C (Windows-verified): uvicorn's own ``Server.serve()`` wraps
itself in ``capture_signals()``, and two Servers in one loop would each
...
(uvicorn's own semantics) so a hung connection cannot block shutdown.
...
``applog.init_logging()`` (attaches the rotating file handler to ``studio.*``
+ ``uvicorn.*``), and both ``uvicorn.Config``s are built with
...
_log = logging.getLogger("studio.serve")
...
def main() -> None:
    ...
    # 3. TLS material (best-effort; never blocks boot).
    tls_paths = None
    if settings.TLS_ENABLED:
        ...
        if tls_paths is None:
            _log.warning(
                ...
                "tls log lines for the reason)",
                settings.HOST, settings.PORT,
            )
    else:
        _log.info("tls disabled by configuration (STUDIO_TLS=0 / [server] tls=false)")

    # 4. Servers. The HTTP server is byte-identical to the legacy
    #    `python -m uvicorn app.main:app --host HOST --port PORT` invocation
    #    (lifespan auto -> on); the HTTPS server adds TLS with lifespan off.
    ...
    if tls_paths is not None:
        ...
        _log.info(
            "listeners starting: http=%s:%s https=%s:%s (ca_regenerated=%s)",
            settings.HOST, settings.PORT, settings.HOST, settings.TLS_PORT,
            tls_paths.ca_regenerated,
        )
    else:
        tls_core.set_serving(False)
