"""Persistent rotating file logging — v1 ``studio/app/core/applog.py`` verbatim,
re-rooted for the v2 relay.

* ``studio2/relay/.runtime/logs/studio2.log`` — UTF-8, rotated at ~5 MB, 5
  backups (``studio2.log.1`` … ``studio2.log.5``), so disk use is capped at
  ~30 MB. A separate file from v1's ``studio.log`` — the two stacks run
  simultaneously and ``RotatingFileHandler`` is not multi-process-safe, so
  they must never share a file.
* Line format: ``2026-06-12 19:30:01.123 INFO  studio2.auth | msg key=value``
  ...
  ``studio2.auth`` / ``studio2.tls`` / ``studio2.serve`` / ``studio2.client``;
  Step 3 adds ``studio2.bridge`` / ``studio2.chat`` / ``studio2.agent``).
* The SAME file handler is attached to ``uvicorn.access`` and
  ...
  too. Their console handlers are NOT touched.
* Console behavior for ``studio2.*`` is preserved: an explicit stderr handler
  at WARNING mirrors logging's lastResort behavior, so attaching the file
  handler does not silence the console.
...
exception (the app simply runs file-log-less), and the logging package itself
already swallows per-record handler errors.

Security: call sites must never log passwords, cookie values, or session
tokens. Client-supplied strings (usernames on FAILED logins, client-shipped
diagnostic events, device-supplied tool results) go through
:func:`sanitize_log_value`, which strips control characters so a crafted
value can never forge log lines (CR/LF injection) and caps the length.
"""
...
LOG_DIR_NAME = "logs"
LOG_FILE_NAME = "studio2.log"
LOG_MAX_BYTES = 5 * 1024 * 1024  # ~5 MB per file
LOG_BACKUP_COUNT = 5  # studio2.log + .1 ... .5 -> ~30 MB cap
...
def sanitize_log_value(value: object, max_len: int = 500) -> str:
    ...
    crafted username/message can never start a forged log line; the result is
    truncated to ``max_len`` characters with an ellipsis marker. Never raises
    — an unstringable object degrades to ``"?"``.
    """
    try:
...
def init_logging() -> None:
    ...
    Called first thing in ``create_app()`` AND in ``serve.main()`` (after
    uvicorn's dictConfig) — the second and every later call is a no-op.

    Wiring:
      * ``studio2`` logger -> level INFO, file handler + a stderr handler at
        WARNING (mirrors the lastResort console behavior),
    ...
    try:
        ...
        file_handler._studio2_applog = True  # type: ignore[attr-defined]

        def _has_ours(lg: logging.Logger) -> bool:
            return any(getattr(h, "_studio2_applog", False) for h in lg.handlers)

        studio2 = logging.getLogger("studio2")
        studio2.setLevel(logging.INFO)
        if not _has_ours(studio2):
            studio2.addHandler(file_handler)
            ...
            studio2.addHandler(console)
        studio2.propagate = False
